...
export default {
    ...
    methods: {
        ...
        validate: function() {
            if (this.isModified) {
                this.$validator.validateAll('experiment-editable-form').then(success => {
                    if (success) {
                        // debug info
                        this.debug('experiment editable form is valid')
                        if (this.objectKeyOrPath.indexOf('.') !== -1) {
                            // it is a path
                            var keys = this.objectKeyOrPath.split('.')
                            var postBody = {
                        ...
                        this.$emit('experiment-editable-updated', postBody, this.type)
                    } else {
                        // debug info
                        this.debug('experiment editable form has errors')
                    }
                })
            } else {
                // debug info
                this.debug('experiment editable not modified')
            }
        }
    },
    computed: {
