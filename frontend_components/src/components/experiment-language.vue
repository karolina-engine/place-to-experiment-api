...
export default {
    ...
    props: {
        ...
        value: { // cannot check for String type because experimentPage@getLanguage returns null for non-existing language
            required: true
        },
        languageKey: {
            required: true,
        ...
        defaultLanguage: { // Optional. If true, the language field will post to the 'default language' API endpoint
            required: false,
            default: false,
            type: Boolean
        },
                    ...
                    this.$validator.validateAll().then(success => {
                        if (success) {
                            ...
                            if (!this.defaultLanguage) {
                                ...
                                this.$emit('experiment-language-updated', languageBody)
                            }
    ...
    computed: {
        isModified: function() {
            if (this.editor) {
                if (this.editor.toLowerCase() === 'sirtrevor' && this.format.toLowerCase() === 'json') {
                    if (this.editorInstance) {
                        // TODO: Need to hook into onChangeEvent for SirTrevor to actually detect isModified
                        SirTrevor.onBeforeSubmit()
                        var jsonContent = this.editorInstance.store.toString()
